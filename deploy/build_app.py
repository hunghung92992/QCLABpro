...
# Lấy toàn bộ tài nguyên của qfluentwidgets (bao gồm cả SVG icons và DLLs)
datas, binaries, hiddenimports = collect_all('qfluentwidgets')

# [FIX] Bổ sung các thư viện ẩn cốt lõi của dự án (Đã vá lỗi thiếu Matplotlib và Resources)
hiddenimports += [
    'app.resources_rc',                   # [QUAN TRỌNG] File chứa Icon compile ở Phase 4.2
    'matplotlib.backends.backend_qtagg',  # [QUAN TRỌNG] Backend để vẽ biểu đồ IQC Chart
    'app.core.config',
    'app.services.sync_manager',          # Sửa lại tên chuẩn từ sync_worker -> sync_manager
    'pandas',
    'openpyxl',
    'jinja2',
...
# Chuẩn bị cú pháp data và binary cho PyInstaller
data_options = [f'--add-data={src}{os.pathsep}{dst}' for src, dst in datas]
# [FIX] Đưa binaries (các file .dll, .so) vào lệnh build
binary_options = [f'--add-binary={src}{os.pathsep}{dst}' for src, dst in binaries]
...
PyInstaller.__main__.run([
    ...
    *icon_option,
    *data_options,
    *binary_options, # [FIX] Nạp DLLs vào bộ cài
